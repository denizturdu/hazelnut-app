...
if 'offer_quality_data' not in st.session_state:
    st.session_state.offer_quality_data = {} 
if 'active_quality_row' not in st.session_state:
    st.session_state.active_quality_row = None
if 'generated_excel_data' not in st.session_state:
    st.session_state.generated_excel_data = None
...
def generate_offer_excel(header_data=None, product_df=None, quality_override=None):
    ...
    headers = [
        ("Date:", "B3", header_data.get("date", "")), 
        ("Offer No:", "D3", header_data.get("offer_no", "")), 
        ("Validity:", "F3", header_data.get("validity", "")),
        ("Customer Name:", "B4", header_data.get("customer", "")), 
        ("Cust. Ref:", "D4", header_data.get("cust_ref", "")), 
        ("Avella Ref:", "F4", header_data.get("avella_ref", "")), 
        ("Payment Terms:", "B5", header_data.get("payment", "")), 
        ("Delivery Addr:", "D5", header_data.get("delivery", ""))
    ]
    for label, cell, val in headers:
        ...
        worksheet.write(input_cell, str(val), input_format)
...
# ==========================================
                ...
                with st.form("price_input_form"):
                    ...
                    if st.form_submit_button("Save Entry"):
                        if p_tombul > 0 and p_cakildak > 0 and p_levant > 0:
                            ...
                            st.warning("Please fill all price fields.")
    ...
    # ==========================
        ...
        with st.form("sube_hazelnut_form"):
            ...
            if reg_type != "Emanet":
                st.metric("Kalan Bakiye", f"{total_val - pay_amount:,.2f} TL")

            if st.form_submit_button("✅ Şube Girişini Kaydet"):
                payload = {
                    "created_by": st.session_state.user['email'], 
                    "status": "Pending Arrival", 
                    "category": hazelnut_cat, 
                    "supplier": supplier, 
                    ...
                    "remaining_balance": total_val - pay_amount, 
                    "count_nylon": cnt_nylon, 
                    "count_jute": cnt_jute, 
                    "count_bigbag": cnt_bigbag, 
                    "weight_sample": w_sample, 
                    "weight_good": w_good, 
                    "weight_shrivelled": w_shriv, 
                    "weight_visible_rotten": w_vis_rot, 
                    "weight_hidden_rotten": w_hid_rot, 
                    "weight_tumor": w_tumor, 
                    "weight_undersize": w_under, 
                    "weight_oversize": w_over, 
                    "moisture": val_moist, 
            ...
            with st.form("fab_hazelnut_form"):
                ...
                if calc_btn:
                    ...
                    calc_inputs = {
                        ...
                        "Görünen Küflü": w_vis_mold, "Gizli Küflü": w_hid_mold, "Görünen Urlu": w_vis_tumor, 
                        "Gizli Urlu": w_hid_tumor, "Kurt Yenikli": w_worm, "Buruşuk İç": w_shriv, 
                        "Limoni": w_lemon, "Vurgun": w_decayed, "Kırık": w_broken, "İkiz": w_twin, 
            ...
            if not pending_df.empty:
                ...
                with st.form("intake"):
                    ...
                    if st.form_submit_button("Onayla"):
                        ...
                        insert_record("intake_log", {
                            "po_id": int(selected_id), 
                            "plate_number": plate, 
                            "waybill_no": waybill, 
                            "received_qty": qty, 
                            "location_in_warehouse": loc, 
                            "created_by": st.session_state.user['email']
                        })
                        insert_record("stock_movements", {
            ...
            if not df_users.empty:
                ...
                if selected_email:
                    ...
                    with st.form("edit_user_perm"):
                        ...
                        st.caption("Erişilebilir Modüller")
        ...
        with tab_mat:
            ...
            if action == "Ekle":
                with st.form("add_mat"): 
                    ...
                    use = g1.text_input("Kullanım")
                    mat = g2.text_input("Materyal")
                    oth = g3.text_input("Diğer")
                    
                    if st.form_submit_button("Kaydet"):
                        insert_record("material_definitions", {
                            "category": cat, "item_name": name, "sales_unit": unit, 
                            "unit_quantity": uq, "notes": nt, "dim_outer": dim_o, 
                            "dim_inner": dim_i, "unit_weight_g": w_g, "use_case": use, 
                            "mat_type": mat, "other_specs": oth
                        })
                        st.success("Eklendi!")
                    ...
                    with st.form("edit_mat"): 
                        ...
                        if st.form_submit_button("Güncelle"):
                            supabase.table("material_definitions").update({
                                "item_name": new_name, "mat_type": emat, "use_case": euse, 
                                "sales_unit": eunit, "notes": enote
